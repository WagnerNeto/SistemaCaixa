<?php
	...
	/*Deletar um Movimento selecionado*/

	if (isset($_GET['acao']) && $_GET['acao'] == 'apagar') {
		$id = $_GET['id'];

		mysqli_query("DELETE FROM lc_movimento WHERE id='$id'");
		echo mysqli_error();
	...
	//Renomear/Editar uma categoria

	if (isset($_POST['acao']) && $_POST['acao'] == 'editar_cat') {
		$id = $_POST['id'];
		$nome = $_POST['nome'];

		mysqli_queryi("UPDATE lc_cat SET nome='$nome' WHERE id='$id'");
		echo mysqli_error();
	...
	if (isset($_GET['acao']) && $_GET['acao'] == 'apagar_cat') {
		...
		$qr=mysqli_query("SELECT c.id FROM lc_movimento m, caixa_cat c WHERE c.id=m.cat && c.id=$id");
		if (mysqli_num_rows($qr)>0){
			header("Location: ?mes=" . $_GET['mes'] . "&ano=" . $_GET['ano'] . "&cat_err=1");
			exit();
		}
		
		mysqli_query("DELETE FROM caixa_cat WHERE id='$id'");
		echo mysqli_error();
	...
	//Editar algum movimento que já foi realizado
	...
	if (isset($_POST['acao']) && $_POST['acao'] == 'editar_mov') {
		...
		mysqli_query("UPDATE lc_movimento SET dia='$dia', tipo='$tipo', cat='$cat', descricao='$descricao', valor='$valor' WHERE id='$id'");
		echo mysqli_error();
	...
	if (isset($_POST['acao']) && $_POST['acao'] == 2) {
		...
		mysqli_query("INSERT INTO lc_cat (nome) values ('$nome')");
	...
	//Adicionar movimento em alguma categoria j� existente
	...
	if (isset($_POST['acao']) && $_POST['acao'] == 1) {
		...
		mysqli_query("INSERT INTO lc_movimento (dia,mes,ano,tipo,descricao,valor,cat) values ('$dia','$mes','$ano','$tipo','$descricao','$valor','$cat')");
	...
	<body style="padding:10px">
		<table cellpadding="1" cellspacing="10"  width="900" align="center" style="background-color:#033">
			<tr>
				...
				<td colspan="2" width="400" align="right" style="background-color:#005B5B;">
			...
			<tr >
				<td colspan="3" >
						...
						<table width="100%">
							<tr>
								<td valign="top">
									...
									<form method="post" action="?mes=<?php echo $mes_hoje?>&ano=<?php echo $ano_hoje?>" name="form2" id="form2" onSubmit="return validaCategoria(form2)">
								...
								<td valign="top" align="right">
									...
									<?php	
										...
										$qr=mysqli_query("SELECT id, nome FROM caixa_cat");
					...
					<div style=" background-color:#F1F1F1; padding:10px; border:1px solid #999; margin:5px; display:none" id="add_movimento">
						...
						//Se a categoria n�o for selecionada para o movimento, o mesmo n�o � realizado
							$qr=mysqli_query("SELECT * FROM caixa_cat");
							if (mysqli_num_rows($qr)==0)
						...
						<form method="post" action="?mes=<?php echo $mes_hoje?>&ano=<?php echo $ano_hoje?>" name="form3" id="form3" onSubmit="validaMovimento(form3)">
			...
			<tr>
				<td align="left" valign="top" width="450" style="background-color:#D3FFE2">
					...
					<?php
						...
						//Código para exibir na tela a somatória das despesas, dos ganhos e o total correspondente ao mês selecionado
						$qr=mysqli_query("SELECT SUM(valor) as total FROM caixa_movimento WHERE tipo=1 && mes='$mes_hoje' && ano='$ano_hoje'");
						$row=mysqli_fetch_array($qr);
						$entradas=$row['total'];
						
						$qr=mysqli_query("SELECT SUM(valor) as total FROM caixa_movimento WHERE tipo=0 && mes='$mes_hoje' && ano='$ano_hoje'");
						$row=mysqli_fetch_array($qr);
						$saidas=$row['total'];
				...
				<td align="left" valign="top" width="450" style="background-color:#F1F1F1">
					<fieldset>
						...
						<?php
							...
							//Código para exibir o total de despesas e ganhos no geral
							$qr=mysqli_query("SELECT SUM(valor) as total FROM lc_movimento WHERE tipo=1 ");
							$row=mysqli_fetch_array($qr);
							$entradas=$row['total'];

							$qr=mysqli_query("SELECT SUM(valor) as total FROM lc_movimento WHERE tipo=0 "); 	
							$row=mysqli_fetch_array($qr);
							$saidas=$row['total'];
		...
		<table cellpadding="5" cellspacing="0" width="900" align="center">
						...
						<form name="form_filtro_cat" method="get" action=""  >
							...
							Filtrar por categoria:  <select name="filtro_cat" onchange="form_filtro_cat.submit()">
								...
								<?php
									...
									$qr=mysqli_query("SELECT DISTINCT c.id, c.nome FROM lc_cat c, lc_movimento m WHERE m.cat=c.id && m.mes='$mes_hoje' && m.ano='$ano_hoje'");
									while ($row=mysqli_fetch_array($qr)){
								?>
			...
			<?php
				...
				//Código para filtrar resultados por categoria
				$filtros="";
				if (isset($_GET['filtro_cat'])){
					if ($_GET['filtro_cat']!=''){	
						$filtros="&& cat='".$_GET['filtro_cat']."'";
					
						$qr=mysqli_query("SELECT SUM(valor) as total FROM lc_movimento WHERE tipo=1 && mes='$mes_hoje' && ano='$ano_hoje' $filtros");
						$row=mysqli_fetch_array($qr);
						$entradas=$row['total'];

						$qr=mysqli_query("SELECT SUM(valor) as total FROM lc_movimento WHERE tipo=0 && mes='$mes_hoje' && ano='$ano_hoje' $filtros");
						$row=mysqli_fetch_array($qr);
						$saidas=$row['total'];
				...
				$qr=mysqli_query("SELECT * FROM lc_movimento WHERE mes='$mes_hoje' && ano='$ano_hoje' $filtros ORDER By dia");
				$cont=0;
				while ($row=mysqli_fetch_array($qr)){
				$cont++;

				$cat=$row['cat'];
				$qr2=mysqli_query("SELECT nome FROM lc_cat WHERE id='$cat'");
					...
					<form method="post" action="?mes=<?php echo $mes_hoje?>&ano=<?php echo $ano_hoje?>">
						...
						<select name="cat">
							...
							<?php
								...
								$qr2=mysqli_query("SELECT * FROM lc_cat");
						...
						<input type="reset" class="input" value="Cancelar" />
						<input type="submit" class="input" value="Alterar" />
					</form> 
		...
		<table cellpadding="5" cellspacing="0" width="900" align="center">
			<tr>
				<td align="right">
					<hr size="1" />
					<em>Caixa - <a href="login.php?sair">Fazer logout</a></em>
				</td>
			</tr>
		</table>
	</body>
